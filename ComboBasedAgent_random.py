...
class ComboBasedAgent_Random(Agent):
    """
...
"""
    ...
    def predict(self, obs):
        ...
        if (distance < self.attack_range and 
            ...
            # Only trigger if not already in a combo.
            possible = []
            if not self.combo_active:
                # Decide which combo to run
                if distance_x > 0:
                    # Opponent is to the right.
                    if opponent_vel[0] < 0.5:
                        possible = [self.combo_horizontal_right_neutral, self.combo_vertical_down_neutral]
                    else:
                        possible = [self.combo_horizontal_right_down]
                elif distance_x < 0:
                    # Opponent is to the left.
                    if opponent_vel[0] > -0.5:
                        possible = [self.combo_horizontal_left_neutral, self.combo_vertical_down_neutral]
                    else:
                        possible = [self.combo_horizontal_left_down]
                else:
                    possible = [self.combo_vertical_neutral_neutral]
            if possible:
                self.combo_sequence = random.choice(possible)
                self.combo_active = True
                self.combo_index = 0
        
        # Execute combo if active.
        if self.combo_active:
            if self.combo_index < len(self.combo_sequence):
                action = self.combo_sequence[self.combo_index]
                self.combo_index += 1
                # When combo finishes, set a cooldown so it doesn't immediately re-trigger.
                if self.combo_index >= len(self.combo_sequence):
                    self.combo_active = False
                    self.combo_cooldown = self.time + 30  
            else:
                self.combo_active = False
                # Cool down
            return action
            ...
            # Recovery mode
                ...
                if player_pos[0] < opponent_pos[0] and opponent_facing == 0:
                  ...
                  and player_dodge_timer <= 0 
                  and player_state not in [state_mapping['AttackState'], state_mapping['StunState']] \
                  and -5.0 < player_pos[0] < 5.0):
                options = []
                if player_grounded:
                    if player_pos[0] < opponent_pos[0]:
                        options = [self.LIGHT_ATTACK_DOWN, self.LIGHT_ATTACK_NEUTRAL, self.LIGHT_ATTACK_RIGHT,
                                self.HEAVY_ATTACK_DOWN, self.HEAVY_ATTACK_NEUTRAL, self.HEAVY_ATTACK_RIGHT]
                    else:
                        options = [self.LIGHT_ATTACK_DOWN, self.LIGHT_ATTACK_NEUTRAL, self.LIGHT_ATTACK_LEFT,
                                self.HEAVY_ATTACK_DOWN, self.HEAVY_ATTACK_NEUTRAL, self.HEAVY_ATTACK_LEFT]
                elif player_aerial:
                    if player_pos[0] < opponent_pos[0]:
                        options = [self.LIGHT_ATTACK_DOWN, self.LIGHT_ATTACK_NEUTRAL, self.LIGHT_ATTACK_RIGHT, GROUNDPOUND]
                    else:
                        options = [self.LIGHT_ATTACK_DOWN, self.LIGHT_ATTACK_NEUTRAL, self.LIGHT_ATTACK_LEFT, GROUNDPOUND]
